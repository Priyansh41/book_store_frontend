...
const Settings = () => {
  ...
  const submitAddress= async()=>{
    const response= await axios.put(`${BASE_URL}/api/v1/update-address`,Value,{headers});
    alert(response.data.message);
  };
  return <>{!ProfileData && <div className='w-full h-[100%] flex items-center justify-center'><Loader/></div>}
  {ProfileData && (
